...
we have **quite different semantics**:
    ...
    **immutability**
        ...
        otherwise we might reuse a path instance used in a previous command which
        might be executed in the future.
...
ergonomic while easy to maintain.
...
Now we can construct an invocation by providing arguments to it. There are two
ways to do so, with slightly different behaviours:

    **call**

        The value provided as argument is used verbatim on the called command. No
        need to think about strange quoting and escaping rules.
    ...
    **slice**
        ...
        shell would do, so one or more arguments can be added to the called
        command.
...
Evaluation of the configured command happens explicitly when we cast it to a
primitive value:
...
>>> int( grep['foo'] )
    executes and returns the exitcode
>>> bool( grep['foo'] )
...
>>> sh('ls *.jpg')
    /bin/sh -e -c 'ls *.jpg'   # globbing is done by the shell
...
to use the :ref:`Lazy application: <= 🚧` operator with a raw multiline literal:
