...
ways to do so, with slightly different behaviors:
    ...
    **slice**
        ...
        Additionally it detects paths and globbing syntax, automatically creating
        :class:`pysh.dsl.Path` and :class:`pysh.dsl.GlobMatcher` instances for
        those.

        >>> ls['*.jpg']
            ls( _['*.jpg'] )
          # ls file1.jpg file2.jpg ...
        >>> tar['-cvf archive.tar imgs/*.{jpg,gif}']
            tar( '-cvf', archive.tar, _['imgs/*.{jpg,gif}'] )
          # tar -cvf archive.tar imgs/file1.jpg imgs/file2.gif ...

        .. Caution::
            There is no parsing of quoted strings on the value, only *whitespace*
            and *globbing syntax* have special meaning. If you need to provide some
            text with quotes use the **call** style.


Each time we *call* or *slice* on a command a **cloned instance is returned** with
the changes, this usually works transparently due to the chained nature of the DSL.
...
Evaluation of the built command happens explicitly when we cast it to a primitive
value:
